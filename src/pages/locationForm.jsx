...
function LocationForm() {
  ...
  const localSchoolOption = "Enrolling in local school for now";
      ...
      .then((res) => {
        ...
        if (res.data?.formData) {
          setFormData({
            ...
            Child1School: res.data.formData.Child1School || "",
            Child2School: res.data.formData.Child2School || "",
            Child3School: res.data.formData.Child3School || "",
            Child4School: res.data.formData.Child4School || "",
            Child5School: res.data.formData.Child5School || "",
  ...
  const getSchoolFieldName = (studentNumber) => {
    return `Child${studentNumber}School`;
  };
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (name.includes("LearningMode")) {
        const schoolFieldName = name.replace("LearningMode", "School");
        if (value !== localSchoolOption) {
          updated[schoolFieldName] = "";
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    for (const child of formData.children) {
      const learningModeField = getLearningModeFieldName(child.studentNumber);
      const schoolField = getSchoolFieldName(child.studentNumber);

      if (
        formData[learningModeField] === localSchoolOption &&
        !String(formData[schoolField] || "").trim()
      ) {
        setError(`Please enter the school name for Child ${child.studentNumber}.`);
        setSubmitting(false);
        return;
      }
    }
    ...
    try {
      await axios.post("/api/location-notice", formData, {
        withCredentials: true,
        headers: {
        ...
        <form onSubmit={handleSubmit}>
          ...
          {formData.children.length > 0 ? (
            formData.children.map((child) => {
              ...
              const schoolFieldName = getSchoolFieldName(child.studentNumber);
              ...
              return (
                ...
                >
                  ...
                  {formData[fieldName] === localSchoolOption && (
                    <div style={{ marginBottom: "0.75rem" }}>
                      <label>{`School your child ${child.studentNumber} will be attending`}</label>
                      <input
                        type="text"
                        name={schoolFieldName}
                        value={formData[schoolFieldName] || ""}
                        onChange={handleChange}
                        className="form-control"
                        required
                      />
...
//     //     withCredentials: true,
//     //   })
//     axios.get("/api/location-form-data", {
//   withCredentials: true,
// })
//       .then((res) => {
//         const detectedTimeZoneRaw =
